...
with st.sidebar:
    ...
    threshold = st.slider("Clustering Threshold", 0.70, 0.99, 0.82, 0.01, help="Cao hơn = cluster chặt hơn")
    sim_filter = st.slider("Similarity Filter", 0.70, 0.99, 0.88, 0.01, help="Keyword dưới ngưỡng tách thành singleton")
...
uploaded = st.file_uploader("📂 Upload file keywords (.xlsx hoặc .csv)", type=["xlsx","xls","csv"])
...
if uploaded:
    ...
    if st.button("🚀 Run Clustering", type="primary", use_container_width=True):
        ...
        with st.spinner("⏳ Loading model + encoding... (lần đầu ~30-60s)"):
            ...
            model = _load()
            embs = model.encode(keywords, batch_size=batch_size, show_progress_bar=False,
        ...
        with st.spinner("⏳ Clustering..."):
            ...
            if N > 3000:
                ...
                conn = kneighbors_graph(embs, n_neighbors=min(15, N-1), metric='cosine',
        ...
        with st.spinner("⏳ Building output..."):
            ...
            def pick_main(members_with_vol):
                if len(members_with_vol) == 1:
                    return members_with_vol[0][0]
        ...
        st.download_button("⬇️ Download Excel", data=buf, file_name=f"Keyword_Clusters_{now}.xlsx",
